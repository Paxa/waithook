...
type SharedSender = Arc<Mutex<WsClientSender<WebSocketStream>>>;
...
pub fn run_server(server_port : u16) {
    ...
    run_broadcast_broker(reciever, broker_subscribers);
    ...
    for connection in ws_server {
        ...
        thread::spawn(move || {
            ...
            if !headers.has::<header::Upgrade>() {
                ...
                listen_and_forward(req_local_ws_sender, channel_reciever, path, client_ip);
                ...
                thread::spawn(move || {
                    for message in ws_receiver.incoming_messages() {
                        ...
                        match message.opcode {
                            ...
                            Type::Ping => {
                                handle_ping_message(message, &local_ws_sender, client_ip);
                            },
                            Type::Pong => {
                ...
                keep_alive_ping(ping_local_ws_sender, client_ip.clone());
            }
        });
    }
}
